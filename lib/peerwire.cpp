...
bool send_handshake(int sockfd, std::string peer_id, std::string info_hash) {
    ...
    // INFO_HASH
    memcpy(offset, info_hash.cstr(), SHA1_LEN);
    offset += SHA1_LEN;

    // INFO_HASH
    memcpy(offset, peer_id.cstr(), PEER_ID_LEN);

    // send message
    if(send(sockfd, msg, HANDSHAKE_MSG_SIZE, 0)) {
        // error
        return false;
    }
    return true;
...
bool send_piece(int sockfd, uint32_t index, uint32_t begin, char *data, size_t n) {
