...
public:
    TrackerConnection(std::string tracker_url);

    // announces client to tracker
    TrackerResponse send_request(TrackerRequest req);

    // checks if we can reannounce to tracker
    // true if the last sent time + interval > current time
    bool interval_expired(int interval);
...
public:
    ...
    string info_hash;   // TODO: url encode

    // urlencoded 20-byte string used as a unique ID for the client,
    string peer_id;     // TODO: url encode
...
public:
    static TrackerResponse parse(map<std::string, BencodeTokenPtr> resp_dict);
